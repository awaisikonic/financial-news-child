  ...
  async function updateMarketData(category) {
    try {
      const response = await fetch(
        `/wp-admin/admin-ajax.php?action=market_ticker&category=${encodeURIComponent(
          category
        )}`
      );
      ...
      marketWrapper.innerHTML =
        '<p class="error">Failed to load market data</p>';
    }
  }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const categoryItems = document.querySelectorAll(
    ".future-section__latest-header-catg-item"
  );
  const latestNewsContainer = document.getElementById("latest-news-container");
  if (toggleDropdown) {
    toggleDropdown.addEventListener("click", () => {
      document
        .querySelector(".future-section__latest-header-catg-list")
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const categoryItems = document.querySelectorAll(
    ".latest-post__featured-section .future-section__latest-header-catg-item"
  );
  ...
  toggleDropdown.addEventListener("click", () => {
    document
      .querySelector(
        ".latest-post__featured-section .future-section__latest-header-catg-list"
      )
      .classList.toggle("active");
  });
